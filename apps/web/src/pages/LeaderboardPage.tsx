...
export function LeaderboardPage() {
  ...
  if (status === "loading")
    return <LeaderboardState message="Loading trail leaders…" />;
  if (status === "error")
    return (
      <LeaderboardState
  ...
  if (!data?.entries.length)
    return (
      <LeaderboardState message="No explorers are ranked yet. Complete an English Point to be the first!" />
    );

  const rows = data.currentPlayer
    ? [...data.entries, data.currentPlayer]
    : data.entries;
  return (
    <section
      className="mx-auto my-8 w-[min(calc(100%-32px),760px)]"
    ...
    >
      <div className="mb-6 text-center">
        ...
        <h1
          className="my-2 text-[clamp(2rem,8vw,3.3rem)]"
          id="leaderboard-title"
        >
          Leaderboard
        </h1>
        <p className="text-muted">
          Scores come from first completions across all ten English Points.
        </p>
      </div>
      <ol className="m-0 grid list-none gap-3 p-0" aria-label="Player rankings">
        {rows.map((entry, index) => (
          ...
          >
            ...
            <img
              className="size-12 rounded-full border border-ink object-cover"
              src={entry.avatarId}
              alt=""
              width="48"
              height="48"
...
function LeaderboardState({
  ...
  return (
    <section
      className="mx-auto my-11 flex min-h-[55vh] w-[min(calc(100%-32px),760px)] flex-col items-center justify-center text-center"
      role="status"
      aria-live="polite"
    >
      ...
      {action && (
        <button
          className="min-h-12 rounded-xl bg-ink px-5 font-black text-white"
          type="button"
          onClick={onAction}
        >
          {action}
